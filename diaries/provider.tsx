...
type Context = {
  diaries: Diary[]; isLoading: boolean; loadError: string | null; mutationError: string | null; canRetryPending: boolean; capability: Capability; canEdit: boolean; canEditMedia: boolean;
  ...
  addItem(diaryId: string, dayId: string, topicId: string, item: NewDiaryItem & { locationSource?: 'PIN_NOW' | 'MAP_SELECTED'; mutationClientRequestId?: string }): Promise<DiaryItem>; attachCoverPhoto(diaryId: string, photoAssetId: string, clientRequestId: string): Promise<void>; moveTopic(diaryId: string, dayId: string, topicId: string, offset: -1 | 1): Promise<void>; moveItem(diaryId: string, dayId: string, topicId: string, itemId: string, offset: -1 | 1): Promise<void>;
  ...
  updateItem(diaryId: string, dayId: string, topicId: string, itemId: string, input: Partial<Pick<DiaryItem, 'manuallyEditedAt'>> & { text?: string; title?: string | null; note?: string | null; caption?: string | null; label?: string | null; url?: string; location?: DiaryCoordinates }): Promise<void>;
};
...
export function DiaryProvider({ children }: PropsWithChildren) {
  ...
  const execute = useCallback(async (intent: DiaryMutationIntent, expectedVersion: number | null) => {
    ...
    switch (intent.kind) {
      ...
      case 'attach_cover': return diaryMutationApi.attachCover(d, p);
      case 'remove_cover': { const { mediaId, ...body } = p; return diaryMutationApi.removeCover(d, String(mediaId), body); }
      case 'reorder_cover': return diaryMutationApi.reorderCover(d, p);
    }
  }, []);
  ...
  const semanticallySatisfied = useCallback((intent: DiaryMutationIntent, detail: DiaryApiDetail) => {
    ...
    if (intent.kind === 'attach_cover') return detail.coverMedia.some((media) => media.clientRequestId === intent.clientRequestId);
    if (intent.kind === 'remove_cover') return !detail.coverMedia.some((media) => media.id === p.mediaId);
    if (intent.kind === 'reorder_cover') return detail.coverMedia.map(({ id }) => id).join('|') === (p.mediaIds as string[]).join('|');
  ...
  const enqueue = useCallback(async (diaryId: string, kind: DiaryMutationKind, payload: Record<string, unknown>, stableClientRequestId?: string) => { const ownerId = activeUser.current; if (!ownerId) throw new ApiError(401, 'mobile_session_required'); let item = outbox.current.items.find((value) => stableClientRequestId ? value.clientRequestId === stableClientRequestId : equivalentDiaryIntent(value, kind, diaryId, payload)); if (!item) { item = { id: requestId(), diaryId, kind, payload, clientRequestId: stableClientRequestId ?? requestId(), expectedVersion: null, attempted: false, conflictRetries: 0, status: 'queued', errorCode: null, createdAt: new Date().toISOString() }; outbox.current = appendDiaryIntent(outbox.current, item); await persistOutbox(ownerId); } else if (!equivalentDiaryIntent(item, kind, diaryId, payload)) throw new ApiError(409, 'idempotency_conflict'); const promise = new Promise<MutationResult>((resolve, reject) => waiters.current.set(item!.id, [...(waiters.current.get(item!.id) ?? []), { resolve, reject }])); void processDiary(ownerId, item.diaryId); return promise; }, [persistOutbox, processDiary]);
  ...
  const value = useMemo<Context>(() => ({
    diaries, isLoading, loadError, mutationError, canRetryPending, capability, canEdit: capability !== 'unsupported' && capability !== 'checking', canEditMedia: capability !== 'unsupported' && capability !== 'checking',
    ...
    async updateDiaryMetadata(id, input) { const { coverPhotoAssetId, coverPhotoAssetIds, ...metadata } = input; if (Object.keys(metadata).length) await enqueue(id, 'update_diary', metadata); if (coverPhotoAssetId === undefined && coverPhotoAssetIds === undefined) return; const desired = [...new Set(coverPhotoAssetIds ?? (coverPhotoAssetId ? [coverPhotoAssetId] : []))].slice(0, 4); for (const media of [...current(id)!.coverMedia].reverse()) if (!desired.includes(media.photoAssetId)) await enqueue(id, 'remove_cover', { mediaId: media.id }); for (const photoAssetId of desired) if (!current(id)!.coverMedia.some((media) => media.photoAssetId === photoAssetId)) await enqueue(id, 'attach_cover', { photoAssetId, position: current(id)!.coverMedia.length }); const ordered = desired.map((assetId) => current(id)!.coverMedia.find((media) => media.photoAssetId === assetId)!.id); if (ordered.join('|') !== current(id)!.coverMedia.map(({ id: mediaId }) => mediaId).join('|')) await enqueue(id, 'reorder_cover', { mediaIds: ordered }); },
    async attachCoverPhoto(id, photoAssetId, clientRequestId) { if (current(id)!.coverMedia.length >= 4) throw new ApiError(422, 'diary_cover_limit'); await enqueue(id, 'attach_cover', { photoAssetId, position: current(id)!.coverMedia.length }, clientRequestId); },
    ...
    async addItem(id, _dayId, topicId, item) { const topic = current(id)!.days.flatMap(({ topics }) => topics).find((value) => value.id === topicId)!; const position = topic.objects.length; const payload = item.type === 'LOCATION' ? { topicId, type: 'PIN', label: item.label, location: { ...item.location, source: item.locationSource ?? 'PIN_NOW' }, position } : item.type === 'NARRATIVE' ? { topicId, type: 'NARRATIVE', title: item.title, text: item.text, position, includeOnMap: item.includeOnMap } : item.type === 'LINK' ? { topicId, type: 'LINK', title: item.title, url: item.url, note: item.note, position, includeOnMap: item.includeOnMap } : item.type === 'PHOTO' ? { topicId, type: 'PHOTO', photoAssetId: item.photoAssetId, caption: item.caption, position, includeOnMap: item.includeOnMap } : item.type === 'EDITORIAL_PLACE' ? { topicId, type: 'EDITORIAL_PLACE', editorialPlaceId: item.editorialPlaceId, titleSnapshot: item.presentationTitle, locationSnapshot: item.location && { ...item.location, accuracyMeters: null }, position, includeOnMap: item.includeOnMap } : { topicId, type: 'PERSONAL_PLACE', personalPlaceCardId: item.personalPlaceCardId, titleSnapshot: item.presentationTitle, locationSnapshot: item.location && { ...item.location, accuracyMeters: null }, position, includeOnMap: item.includeOnMap }; const completed = await enqueue(id, 'create_object', payload, item.mutationClientRequestId); const objectId = completed.detail.days.flatMap(({ topics }) => topics).flatMap(({ objects }) => objects).find((value) => value.clientRequestId === completed.clientRequestId)?.id; const result = diaryDetailView(completed.detail); const created = result.days.flatMap(({ topics }) => topics).flatMap(({ items }) => items).find((value) => value.id === objectId); if (!created) throw new ApiError(500, 'diary_created_object_missing'); return created; },
    ...
    async updateItem(id, _dayId, _topicId, objectId, input) { const { manuallyEditedAt: _, note, caption, label, location, ...rest } = input; await enqueue(id, 'update_object', { objectId, ...rest, ...(note !== undefined ? { text: note } : {}), ...(caption !== undefined ? { text: caption } : {}), ...(label !== undefined ? { title: label } : {}), ...(location ? { location: { ...location, source: 'MAP_SELECTED' } } : {}) }); },
