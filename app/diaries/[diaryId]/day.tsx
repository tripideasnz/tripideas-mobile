...
function CompletedItem({ date, diaryId, item }: { date: string; diaryId: string; item: DiaryItem }) {
  ...
  if (item.type === 'PERSONAL_PLACE') return <SavedPlaceObject available={item.available !== false} kind="personal" title={item.presentationTitle} onPress={item.available === false ? undefined : () => router.push({ pathname: '/personal-place-cards/[cardId]', params: { cardId: item.personalPlaceCardId, mode: 'view', origin: 'diary', diaryId, date } })} />;
  return null;
}
...
function CompletedTopic({ date, diaryId, onEdit, topic }: { date: string; diaryId: string; onEdit?: () => void; topic: DiaryTopic }) {
  ...
  for (let index = 0; index < ordered.length;) {
    ...
    content.push(<View key={`photos:${photos.map(({ id }) => id).join('|')}`}><DiaryPhotoGrid assetIds={photos.map(({ photoAssetId }) => photoAssetId)} label={`${topic.title || 'Diary Topic'} photos`} />{photos.filter(({ caption }) => Boolean(caption)).map(({ caption, id }) => <AppText key={`caption:${id}`}>{caption}</AppText>)}</View>);
  }
...
function CollapsedItem({ item }: { item: DiaryItem }) {
  ...
  if (item.type === 'PERSONAL_PLACE') return <SavedPlaceObject available={item.available !== false} kind="personal" showLabel={false} title={item.presentationTitle} onPress={item.available === false ? undefined : () => router.push({ pathname: '/personal-place-cards/[cardId]', params: { cardId: item.personalPlaceCardId, mode: 'view', origin: 'diary', diaryId, date } })} />;
...
export default function DiaryDayScreen() {
  ...
  const attachPendingPhoto = useCallback(async (pending: PendingDiaryPhotoAttachment, photoAssetId: string) => { if (pending.target.kind !== 'object') return; await actionsRef.current.addItem(pending.target.diaryId, pending.target.dayId, pending.target.topicId, { type: 'PHOTO', photoAssetId, caption: null, contentOrigin: 'USER_OWNED', includeOnMap: false, mutationClientRequestId: pending.clientRequestId }); }, []);
  useEffect(() => { if (!session?.userId || !diary?.id) return; void resumeDiaryPhotos(session.userId, diary.id, 'object', attachPendingPhoto).then(({ pendingCount }) => setPendingPhotoCount(pendingCount)); }, [attachPendingPhoto, diary?.id, session?.userId]);
  ...
  const openCapture = (topicId: string, action: DiaryObjectAction) => {
    ...
    if (action === 'Photo') { if (!day || !session?.userId || photoBusy) return; void pickPhotosForUpload().then(async (selected) => { if (!selected.length) return; setPhotoBusy(true); setStructuralError(null); const result = await addDiaryPhotos(session.userId, { kind: 'object', diaryId: diary.id, dayId: day.id, topicId }, selected, attachPendingPhoto); setPendingPhotoCount(result.pendingCount); if (result.errors.length) setStructuralError('Some photo uploads paused. Retry when connectivity returns.'); }).catch(() => setStructuralError('Photos could not be added. Check your connection and try again.')).finally(() => setPhotoBusy(false)); return; }
  ...
  return <View {...swipe.panHandlers} style={{ backgroundColor: Palette.background, flex: 1 }}>
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}><SavedObjectFocusScope contentRef={scrollContentRef} scrollRef={scrollRef}><ScrollView ref={scrollRef} innerViewRef={scrollContentRef} contentContainerStyle={{ gap: Space.xxl, padding: Screen.gutter, paddingBottom: 112 }} keyboardDismissMode="interactive" keyboardShouldPersistTaps="handled">
      ...
      {photoBusy ? <AppText color={Palette.textMuted} variant="caption">Uploading selected photos…</AppText> : null}
      {pendingPhotoCount ? <AppButton label={`Retry ${pendingPhotoCount} pending ${pendingPhotoCount === 1 ? 'photo' : 'photos'}`} size="compact" variant="secondary" onPress={() => session?.userId ? void resumeDiaryPhotos(session.userId, diary.id, 'object', attachPendingPhoto).then(({ pendingCount }) => setPendingPhotoCount(pendingCount)) : undefined} /> : null}
      ...
      {[...(day?.topics ?? [])].sort((a, b) => a.position - b.position).map((topic) => activeTopicId !== topic.id ? <CompletedTopic date={date} diaryId={diary.id} key={topic.id} onEdit={canEdit ? () => { setCapture(null); setExpandedItemId(null); setFocusTopicId(null); setStructuralError(null); setActiveTopicId(topic.id); } : undefined} topic={topic} /> : <SavedAutosaveScope key={topic.id}>{(flush) => <View onLayout={(event) => revealNewTopic(topic.id, event.nativeEvent.layout.y)} style={{ backgroundColor: Palette.surfaceMuted, borderRadius: Radius.card, gap: Space.sm, padding: Space.md }}>
        ...
        {[...topic.items].sort((a, b) => a.position - b.position).map((item, index, ordered) => { if (item.type === 'PHOTO') { if (ordered[index - 1]?.type === 'PHOTO') return null; const photos = ordered.slice(index, ordered.findIndex((candidate, candidateIndex) => candidateIndex > index && candidate.type !== 'PHOTO') < 0 ? ordered.length : ordered.findIndex((candidate, candidateIndex) => candidateIndex > index && candidate.type !== 'PHOTO')).filter((candidate): candidate is Extract<DiaryItem, { type: 'PHOTO' }> => candidate.type === 'PHOTO'); const ids = photos.map(({ id }) => id); return <DragReorderRow key={`photos:${ids.join('|')}`} canMoveUp={index > 0} canMoveDown={index + photos.length < ordered.length} label={photos.length === 1 ? 'Photo' : 'Photo group'} header={<AppText color={Palette.textMuted} variant="label">PHOTOS</AppText>} onMove={(offset) => { const next = moveRunIds(topic.items, ids, offset); if (next) void actions.reorderItems(diary.id, day!.id, topic.id, next); }}><DiaryPhotoGrid assetIds={photos.map(({ photoAssetId }) => photoAssetId)} bottomMargin={0} label={`${topic.title || 'Diary Topic'} selected photos`} onRemoveAsset={(assetId) => { const photo = photos.find(({ photoAssetId }) => photoAssetId === assetId); if (photo) void actions.deleteItem(diary.id, day!.id, topic.id, photo.id); }} />{photos.map((photo, photoIndex) => <DiaryAutosaveField accessibilityLabel={`Photo ${photoIndex + 1} caption`} key={`caption:${photo.id}`} maxLength={10_000} placeholder="Optional caption" value={photo.caption ?? ''} onSave={(caption) => actions.updateItem(diary.id, day!.id, topic.id, photo.id, { caption: caption.trim() || null })} />)}</DragReorderRow>; } const compact = item.type === 'PERSONAL_PLACE' || item.type === 'EDITORIAL_PLACE'; return <DiaryObjectEditorShell key={item.id} label={objectLabel(item)} editable={!compact} expanded={!compact && expandedItemId === item.id} collapsed={<CollapsedItem item={item} />} canMoveUp={item.position > 0} canMoveDown={item.position < topic.items.length - 1} onCollapse={() => { setFocusItemId(null); setExpandedItemId(null); }} onExpand={() => { if (!compact) { setFocusItemId(null); setExpandedItemId(item.id); } }} onMove={(offset) => { setExpandedItemId(null); void actions.moveItem(diary.id, day!.id, topic.id, item.id, offset); }} onRemove={() => { if (expandedItemId === item.id) setExpandedItemId(null); void actions.deleteItem(diary.id, day!.id, topic.id, item.id); }}>
          ...
          {item.type === 'PERSONAL_PLACE' ? <SavedPlaceObject available={item.available !== false} kind="personal" showLabel={false} title={item.presentationTitle} onPress={item.available === false ? undefined : () => router.push({ pathname: '/personal-place-cards/[cardId]', params: { cardId: item.personalPlaceCardId, mode: 'view', origin: 'diary', diaryId: diary.id, date } })} /> : item.type === 'EDITORIAL_PLACE' ? <SavedPlaceObject showLabel={false} title={item.presentationTitle} onPress={() => router.push({ pathname: '/place/[slug]', params: { slug: item.editorialPlaceId, editorialPlaceId: item.editorialPlaceId, origin: 'diary', diaryId: diary.id, date } })} /> : null}
